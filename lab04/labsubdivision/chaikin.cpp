...
{
    //Extend and edit this code
    Curve.clear();
    std::vector<vec3> currentInput;
    ...
    while (zValue < 10) {
        for (size_t i(0); i < ControlPolygon.size(); i++) {
            ...
            for (size_t j(0); j < NumPointsPerPolygonLeg; j++) {
                ...
                Curve.push_back(0.75 * LeftPoint + 0.25 * RightPoint); //for the even values

                Curve.push_back(0.25 * LeftPoint + 0.75 * RightPoint); //for the odd values
            }
        }

        currentInput = Curve;  // update the new input with the values of the modified curve
        zValue = zValue + 1; //adding an iteration for it

        goToInit = MinNumDesiredPoints - 50;
        if (goToInit > 0) {
            CornerCutting(currentInput, goToInit, Curve);
        }
    }
}
