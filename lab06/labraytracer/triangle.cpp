...
bool Triangle::closestIntersection(const Ray& ray, double maxLambda,
    ...
    vec3 tr = ray.getDirection(); //direction of the ray
    vec3 pr = ray.getOrigin(); //origin of the ray
    vec3 uvw(0, 0, 0);

    vec3 p0 = mVertices[0]; // vertices of the triangle plane
    vec3 p1 = mVertices[1];
    vec3 p2 = mVertices[2];

    // Lecture 08 - Raytracing
    // Page 18 Ray-Triangle intersection
    vec3 t1 = p1 - p0; // ray direction between point p1 and p0
    vec3 t2 = p2 - p0; // ray direction between point p2 and p0

    // n = t1 x t2
    vec3 n = glm::normalize(glm::cross(t1, t2)); //

    double lambda = -1; //initialize lambda value
    ...
    vec3 p = ray.pointOnRay(lambda); // actual intersection point between the ray and the triangle

    // check if point inside triangle geometrically
    // https://ppt-online.org/703109 slide 25 (University of Virginia 445)
    vec3 cp1 = cross(p2 - p1, p - p1); //cross product between verties of the triangle
    ...
    if (dot(cp1, cp2) < 0) return false; // check if the point is inside the triangle
    if (dot(cp3, cp4) < 0) return false;
    if (dot(cp5, cp6) < 0) return false;

    intersection = RayIntersection(ray, shared_from_this(), lambda, ray.pointOnRay(lambda), n, uvw);
    return true;
}
